...
public class LogHandler extends EditResponseHttpHandler<ResultView> {
	...
	protected ResultView doRun() throws Exception {
		...
		if (method.equals("GET")) {
			...
			try {
				logger.info(count + "  +===========" + new Date() + "  ============ post http!!!");
				...
				System.out.println("MSserver-----" + count + "+++" + new Date() );
		...
		if (paramMap != null && paramMap.get("systime") != null) {
			try {
				paramMap.put("date_time", DateUtil
						.formatDate(new Date(Long.valueOf(paramMap.get("systime").toString())), "yyyy-MM-dd HH:mm:ss"));
			} catch (Exception e) {
				logger.error(e);
			}
