...
public class MyCache {
	private static final ESLogger logger = Loggers.getLogger(MyCache.class);
	private static Long LiveTime = Long.parseLong(Config.get().get("mycash.live.time", "3600000"));// 有效时间一小时
	private static Long internalTime = Long.parseLong(Config.get().get("mycash.clean.time", "3600000"));;// 每分钟定期清理时间
	private static TreeMap<String, CashUnit> cacheMap = new TreeMap<String, CashUnit>();
	private static MyCache MYCASH;

	public static final MyCache getInstance() {
		if (MYCASH == null) {
			...
			public void run() {
				try {
					synchronized (cacheMap) {
						int count = 0;
						logger.info("+++++ Map定期清理 +++++ Size = " + cacheMap.size() + new Date().toString());
						...
						while (iter.hasNext()) {
							...
							if (cacheExpired(cache)) {
								...
								iter.remove();
								// cacheMap.remove(key);
								logger.info("+++++ Map定期清理 key = " + key);
							}
						}
						logger.info("+++++ Map总数KEY = " + cacheMap.size() + ", count = " + count + new Date().toString());
					}
				} catch (Exception e) {
					logger.error(e);
				} catch (Throwable e) {
	...
	public static void setInternalTime(Long internalTime) {
		MyCache.internalTime = internalTime;
	}
