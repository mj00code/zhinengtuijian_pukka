 ...
 * 
 */
public class NewsCategoryCache {
	private static final ESLogger logger = Loggers.getLogger(NewsCategoryCache.class);
	private static Long LiveTime = Long.parseLong(Config.get().get("newscategory.live.time", "3600000"));// 有效时间一小时
	private static Long internalTime = Long.parseLong(Config.get().get("newscategory.clean.time", "3600000"));;// 每分钟定期清理时间
	private static TreeMap<String, CashUnit> cacheMap = new TreeMap<String, CashUnit>();
	private static NewsCategoryCache MYCASH;

	public static final NewsCategoryCache getInstance() {
		if (MYCASH == null) {
			synchronized (NewsCategoryCache.class) {
				if (MYCASH == null) {
					MYCASH = new NewsCategoryCache();
				}
			}
		}
		return MYCASH;
			...
			public void run() {
				try {
					synchronized (cacheMap) {
						int count = 0;
						logger.info("+++++ news category Map定期清理 +++++ Size = " + cacheMap.size() + new Date().toString());
						...
						while (iter.hasNext()) {
							...
							if (cacheExpired(cache)) {
								...
								iter.remove();
								cacheMap.remove(key);
								logger.info("+++++ news category Map定期清理 key = " + key);
							}
						}
						logger.info("+++++ news category Map总数KEY = " + cacheMap.size() + ", count = " + count + new Date().toString());
					}
				} catch (Exception e) {
					logger.error(e);
				} catch (Throwable e) {
	...
	public static void setInternalTime(Long internalTime) {
		NewsCategoryCache.internalTime = internalTime;
	}
